...
# 🍽️ WPF Cafeteria Project

**WPF_Cafeteria_Project**는 **WPF(Windows Presentation Foundation)**를 활용하여 개발된 식당/카페테리아 관리 시스템입니다.  
이 시스템은 주문 처리, 메뉴 관리, 테이블 상태 모니터링, 결제 및 사용자 인증 등 식당 운영에 필요한 **핵심 기능**을 제공합니다.  
또한 사용자 친화적인 GUI와 **TCP 통신** 기반의 구조를 통해 손님, 주방, 관리자 간의 효율적인 데이터 흐름을 지원합니다.
...
- **🪑 테이블 관리**  
  - 테이블 실시간 상태 표시 (빈 테이블 / 사용 중 / 결제 대기)  
  - 테이블 간 이동, 합석 기능 지원  

- **💳 결제 시스템**  
  - 현금, 카드 등 다양한 결제 수단 지원  
  - 영수증 출력 기능 포함  
...
- **📡 TCP 통신 기반 3개 뷰 구성**  
  - 손님 뷰: 주문 입력  
  - 주방 뷰: 주문 수신 및 재고 처리  
  - 관리자 뷰: 실시간 매출 및 재고 현황 파악  

---

## 🍜 메뉴 구성

총 **4개의 메뉴**로 구성되며, 각 메뉴는 **고유의 가격**과 **소비되는 재료**를 바탕으로 주문 및 재고 관리가 이루어집니다.
...
|------|------|
| **Frontend** | WPF (XAML, C#) |
| **Backend** | (예: C# .NET Core Web API, Python Flask 등) |
| **Database** | (예: SQLite, SQL Server, MySQL 등) |
| **통신 방식** | TCP 소켓 통신 |
...
| 팀원 | 담당 기능 |
|------|-----------|
| **[팀원 1 이름]** | 주문 관리 및 결제 시스템 개발 |
| **[팀원 2 이름]** | 메뉴 관리 및 데이터베이스 연동 |
| **[팀원 3 이름]** | 테이블 관리 및 GUI/UX 디자인 |
| **[팀원 4 이름]** | 사용자 인증, 권한 관리, 보고서 기능 구현 |
...
> 원한다면 여기에 각 뷰(손님, 주방, 관리자)의 UI 스크린샷을 첨부해보세요!
...
2. DB 연결 문자열 확인 및 설정
3. 각 뷰(Client)별로 별도 실행
4. TCP 서버 실행 후 클라이언트 실행
